...
var JeelizResizer = (function(){
  ...
  function update_sizeCanvas(){
    console.log("in update_sizeCanvas()");
    const menuBarHeight = window.location.href.split('?')[1];
    ...
    _whCanvasPx = [
      ...
      window['innerHeight'] + (menuBarHeight * 1)
    ];
  ...
  function resize_canvasToFullScreen(){
    //get number after "?" in URL. This was sent by background.js.
    const menuBarHeight = window.location.href.split('?')[1];
    //_whCanvasPx = [window['innerWidth']+190, window['innerHeight'] + (menuBarHeight * 1)];
    if (document.fullscreenElement) {
      _whCanvasPx = [window['innerWidth'], window['innerHeight'] + (menuBarHeight * 1)];
    } else {
      _whCanvasPx = [window['innerWidth'] + 190, window['innerHeight'] + (menuBarHeight * 1)];
    }
    console.log("in resize_canvasToFullScreen");
    if (_isInvFullscreenWH){
      _whCanvasPx.reverse();
    }
    _domCanvas.setAttribute('width',  _whCanvasPx[0]);
    _domCanvas.setAttribute('height', _whCanvasPx[1]);
    console.log("canvas width: " + _whCanvasPx[0]);
    console.log("canvas height: " + _whCanvasPx[1])
  }
